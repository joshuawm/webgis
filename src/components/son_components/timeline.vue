<template>
  ...
  <div id="timeline">
      <div >{{notice}}</div>
      <div v-if="active" class="blogContent" v-for="blog in myBlog" >
        <span></span>
        <p style="font-weight:bold;font-size:15px;">{{blog.userName}}</p>
        <p style="font-size:4px;color: #545c64;">于{{blog.data}}发布于<span style="font-weight: bold; font-size: 5px;">{{blog.attrName}}</span></p>
          <div class="blogContentDetail">
          <p style="font-family:'Microsoft YaHei;font-size:20px;';">{{blog.content}}</p>
...
export default {
    ...
    async initialData(){
      ...
      let requestsURl
      if(this.id===-1){
        requestsURl="http://121.5.235.15/api/v2/zhouyou/_table/yb?fields=*&order=yb_id%20DESC"
      }else if(this.id>=0){
       ...
       for (let index = 0;index<data.length;index++){
        if(lastData === data[index].yb_date){
            myBlog[myBlog.length-1].image.push(`${baseURl}${data[index].yb_imgs}`)
        }else{
            let blog={
       ...
       //处理userId获取userName
        for (let key in searchbox.user){
            let res = await axios.get(`http://121.5.235.15/api/v2/zhouyou/_table/Users?fields=user_name&filter=user_id=${key}`,{
                params:{
                    api_key:'956eed8e98667eca2722be6afc37e123212466565cab5df2f7e653d206f3e3c0'
                }
            })
      ...
      for (let key in searchbox.attr){
        let res = await axios.get(`http://121.5.235.15/api/v2/zhouyou/_table/attractions?fields=attraction_name&filter=attraction_id=${key}`,{
          params:{
            api_key:'956eed8e98667eca2722be6afc37e123212466565cab5df2f7e653d206f3e3c0'
          }
        })
