...
import { pop } from 'shelljs/commands';
  ...
  methods:{
    ...
    initialALayer(){
      ...
      this.map.on("click","attractionLayer",(e)=>{
        new mapboxgl.Popup({anchor:"right",className:'pop-up'}).setLngLat([e.lngLat.lng,e.lngLat.lat])
          .setHTML("<div id='popupcontent'></div>").addTo(this.map)
        let popupinfo={
